...
- **Fix: la app quedó colgada 27hs (504) sin que el watchdog de HA la reiniciara** (`config.yaml`, `rootfs/app/main.py`, `rootfs/app/db.py`, `rootfs/app/scrapers_db.py`): el 26/08 el proceso se congeló por completo (ni un log más, ni HTTP ni jobs del scheduler) hasta el reinicio manual del 27/08. Dos causas de fondo:
  - `_conn()` (en `db.py` y `scrapers_db.py`) hace `lock.acquire()` sin timeout sobre el `RLock` que serializa todos los writes a SQLite del proceso — llamado sincrónicamente desde coroutines async, si ese lock queda tomado por lo que sea, el event loop entero se congela para siempre (no solo el caller). Ahora usa `lock.acquire(timeout=DB_LOCK_TIMEOUT)` (30s) y levanta `TimeoutError` si no lo consigue, para fallar rápido en vez de trabar toda la app indefinidamente.
  - El `watchdog` del addon estaba en `tcp://[HOST]:[PORT:8000]` — Supervisor solo hace un connect TCP crudo, que el kernel acepta igual aunque el event loop de Python esté completamente colgado (mismo motivo por el que el proxy devolvía 504: conecta pero nunca responde), así que nunca detectó el problema. Se agrega `GET /health` (sin sesión ni DB) y se cambia el watchdog a `http://[HOST]:[PORT:8000]/health`, que exige una respuesta HTTP real dentro de un timeout — Supervisor chequea cada 120s y reinicia el addon solo tras 2 fallos consecutivos.

## 1.2.80

- **Fix: el detalle por chip de "Presupuesto vs Real" no cerraba con el total del padre** (`static/app.js`): `stats_presupuesto_vs_actual` (`db.py`) suma al `gastado` del padre tanto sus subcategorías como los gastos categorizados directamente en él (sin subcategoría) — pero `_drawBudgetChart()` al hacer drill-down de un chip solo graficaba las subcategorías, así que ese monto "directo al padre" quedaba invisible y el total de la vista general no coincidía con la suma de las barras del detalle (ej. categoría "Viajes"). Se agrega una barra extra `"<categoría> (sin subcat.)"` con `gastado = gastado_padre - Σ gastado_hijos` cuando ese resto es > $0,5, para que el drill-down siempre sume el mismo total que se ve en "Todas".
...
- **Fix: falta el ícono ↺ de refresh manual en las tarjetas (AMEX, BBVA, Galicia)** (`static/app.js`, `static/style.css`): el botón "Forzar actualización ahora" solo existía en `renderSaldos()` (fila superior). Como las tarjetas vienen con `activa=0` por defecto, en la práctica solo viven en el widget de vencimientos/tarjetas, que nunca tuvo ese botón — por eso las cuentas de banco podían forzar un scrape desde el chip y las tarjetas no. Se agregó el mismo patrón (`saldo-chip-refresh`, ícono-solo en la esquina, `runCuentaFromChip`) a las cards del widget de tarjetas, con `position:relative` en `.venc-chipwrap` para anclarlo. Verificado en preview: con una cuenta-tarjeta activa y con `scraper_instance_id` asignado, el ícono aparece y dispara el mismo refresh que en saldos; al desactivar la cuenta desaparece de ambos widgets.
...
- **Migración `dedup_scraper_gastos_v2`: limpieza automática de gastos duplicados de scraper** (`db.py`): corre sola en el próximo login/arranque de cada usuario (como toda migración de este archivo). Encontré, en una DB real exportada por el usuario, 13 gastos duplicados (`archivo_origen='scraper'`) por ~$4.531.000 ARS inflados — el peor caso, un retiro de $400.000 en `bbva_cuenta` reimportado 11 veces entre el 17/07 y el 08/08. Repliqué el bug con los datos reales: el dedup por saldo de `insert_movimientos_raw` (existe desde v0.8.35) rechaza correctamente una segunda inserción idéntica cuando se prueba aislado, así que la causa raíz de por qué falló en producción (¿carrera del scheduler?) queda sin identificar — no pude confirmarla porque `app_log` solo retiene el día en curso. La migración: (1) repite el dedup exacto (misma fuente/fecha/monto/moneda/descripción) que ya hacía `dedup_scraper_gastos_v1` (v0.3.61) pero que era de una sola vez y no cubría lo acumulado desde entonces; (2) además funde grupos donde la descripción cambió de mayúsculas o le agregaron un sufijo (mismo bug que se corrigió en ingesta para bbva_mc/visa en v1.2.73), solo cuando *todos* los miembros del grupo encajan como prefijo case-insensitive del más largo — si no, no toca nada, para no fusionar transacciones distintas que coinciden en importe/fecha. Conserva la fila más informativa y hereda la categoría de un duplicado si la que sobrevive no tenía. No toca gastos con `archivo_origen` de un PDF de resumen (esos son otro caso, fuera de alcance acá). Probado extremo a extremo contra la DB real: 2051→2038 gastos, idempotente (correr la migración de nuevo no borra nada más).

## 1.2.74

- **Indicador de origen (scraping/resumen/manual) en la tabla de Gastos** (`static/app.js`, `static/style.css`): cada fila de la tabla Gastos muestra ahora un icono chico junto al badge de fuente indicando de dónde salió ese gasto — 🤖 Scraping (día a día, período abierto), 📄 Resumen de cuenta (título con el nombre del PDF, auto-descargado o subido a mano) o 🖊 Cargado a mano. Usa la columna `archivo_origen` que ya existía en `gastos` (`'scraper'` / `'manual'` / nombre de archivo) pero no se mostraba en la UI. De paso: confirmado que el resumen ya tiene prioridad sobre el scraping — `consolidate_scraper_duplicates()` corre después de cada importación de PDF (auto-descarga en `bbva.py`/`amex.py` y subida manual en `routes/upload.py`) para `amex`/`bbva_mc`/`bbva_visa`/`bbva_cuenta`, borra el gasto-scraper duplicado y deja el del PDF — no fue necesario tocar esa lógica, ya estaba bien.

## 1.2.73

- **Fix: duplicados diarios en tarjetas de crédito BBVA (bbva_mc/bbva_visa)** (`scrapers_db.py`): `insert_movimientos_raw` generaba una fila nueva por cada corrida cuando la `concept` del consumo cambiaba de mayúsculas o le agregaban un sufijo entre el momento de autorización y el de liquidación (ej. `"PERSONAL"` → `"PERSONAL PERSFLOW49010001 0"`, `"HOYTS"` → `"Hoyts"`). Causa raíz: `bbva_tarjetas.py` no trae ningún identificador único de transacción en `raw_data` (a diferencia de BBVA débito con `numero_operacion`), así que el dedup dependía de un match exacto de descripción que se rompía con esos cambios — y el set de descripciones "genéricas" (`_GENERIC_DESCS`/`_GENERIC_PREFIXES`) está armado para BBVA débito, no para conceptos de tarjeta como "PERSONAL"/"HOYTS". Se agrega un nuevo cross-run check: mismo día + mismo monto + mismo moneda + fuente, con el monto único ese día (misma regla de seguridad que ya usan los otros checks de esta función), y descripción case-insensitive prefijo de la otra en cualquier dirección → se conserva la más larga/específica. Probado con un script standalone que reproduce ambos casos del reporte y confirma 0 duplicados, y que dos consumos genuinamente distintos con igual monto/día (ej. dos compras de $5.000 en comercios distintos) NO se fusionan.
...
- **SSO/OIDC opcional vía authentik** (`sso_config.py` nuevo, `oidc.py` nuevo, `auth.py`, `routes/auth.py`, `main.py`, `config.yaml`, `run.sh`): agrega login single sign-on con Authorization Code flow (Authlib), pensado como frontend de auth con el add-on `authentik` de este mismo repo. Convive con el login local (email/password + `admin_password`) — cuando `oidc_enabled` está prendido y `oidc_issuer`/`oidc_client_id`/`oidc_client_secret` están completos aparece un botón "Iniciar sesión con SSO" en `/auth/login`, pero el form local sigue funcionando igual (fallback si authentik está caído). El callback (`/auth/sso/callback`) valida el token OIDC, exige que el email termine en `@allowed_domain` (o sea el admin), auto-provisiona el usuario local en su primer login (`ensure_sso_user` — password aleatorio inutilizable, solo para que aparezca en el panel de admin) y arma la misma sesión que usa el login local (`stoken` vía `issue_session_token`), así que todo el resto del código (middleware de contexto per-usuario, `require_auth`, admin panel) no se entera de que el login fue por SSO. El rol admin usa el mismo criterio que hoy: `email == admin@<allowed_domain>`. La `redirect_uri` se arma leyendo `X-Forwarded-Proto`/`X-Forwarded-Host` (no `request.url`) porque uvicorn corre detrás de Nginx Proxy Manager sin terminar TLS. No se eligió forward-auth (headers de proxy) porque exigiría confiar ciegamente en un header sin firmar si alguien accede directo al puerto del add-on sin pasar por el proxy.
...
- **EXPERIMENTAL: base del add-on pasa de Alpine a Debian + Chrome real (arm64/amd64)** (`Dockerfile`, `build.yaml`, `scrapers/base.py`): Google liberó builds arm64 de Chrome real en jul 2026 (antes solo existía Chromium open-source para ARM, que Akamai rechazaba de raíz — ver [[project-amex-scraper-akamai-rpi]]). Chrome for Testing todavía NO publica chromedriver arm64 oficial, pero Debian trixie empaqueta `chromium-driver` en ~v150, un solo major contra el Chrome ~v151 recién liberado — dentro de lo que Selenium tolera. Se cambia `build_from` de `ghcr.io/home-assistant/{arch}-base` (Alpine) a `ghcr.io/home-assistant/base-debian:trixie` (multi-arch, con bashio/s6-overlay intactos — `run.sh` no cambia) y el `Dockerfile` instala `chromium-driver` (apt) + Google Chrome real (.deb directo de `dl.google.com`, detectando arch con `dpkg --print-architecture`). `CHROMIUM_BIN`/`CHROMEDRIVER_BIN` apuntan a los nuevos binarios; el resto del código de scraping no cambia (ya leía estas rutas por ENV var). Objetivo: que AMEX/BBVA puedan volver a correr **local en la RPi** sin depender del WebDriver remoto a una Mac. **Sin probar en la RPi real todavía** (no hay Docker disponible en el entorno donde se hizo este cambio) — la primera validación real es reconstruyendo el add-on ahí. Si Akamai lo sigue bloqueando después de este cambio, sospechar del spoofing de fingerprint en `_create_local_driver` (UA hardcodeado a "Chrome/124" + `navigator.platform` forzado a Win32): tiene sentido para Chromium tratando de parecer un browser real, pero con Chrome real de Linux podría ser contraproducente (inconsistencia entre el UA/versión spoofeados y las Client Hints reales que Chrome manda por HTTP).

## 1.2.65

- **DB: lock de escritura por proceso — elimina "database is locked" de raíz** (`db.py`, `scrapers_db.py`): subir el `busy_timeout` (1.2.64) solo reducía la frecuencia del error; SQLite sigue permitiendo un solo writer a la vez, así que dos threads del mismo proceso (requests de la API + jobs del scheduler de scrapers) escribiendo al mismo tiempo seguían pudiendo competir y perder. Se agrega un `threading.RLock` por archivo de DB (`_get_db_lock`, registrado en `db.py`) que `_conn()` adquiere ANTES de abrir la conexión — los writes ahora se hacen cola en vez de competir, así el error deja de poder ocurrir por contención interna. `scrapers_db.py` (que tenía su propia implementación separada de `_conn()`, con timeouts distintos) ahora importa el mismo registro de locks para quedar mutuamente excluido con `db.py` contra el mismo archivo. Se usa RLock (no Lock) porque, con ~89+18 usos de `with _conn()` entre ambos archivos, es inevitable que alguna función que ya tiene una conexión abierta llame a otra que también abre la suya (mismo thread) — un Lock plano haría deadlock ahí; el RLock permite la reentrada del mismo thread sin trabarse. Validado con un test de 60 escrituras concurrentes (mitad `db._conn()`, mitad `scrapers_db._conn()`, 60 threads): 0 errores, 0 deadlocks.

## 1.2.64

- **Scraper BBVA: soporte de WebDriver remoto (Mac), igual que AMEX** (`scrapers/base.py`, `scrapers/amex.py`, `scraper_credentials.py`): BBVA (`online.bbva.com.ar`) también corre atrás de Akamai BotManager (ver docstring de `scrapers/bbva.py`) y viene fallando el login hace dos días con "tras submit seguimos en /login/ — posibles credenciales inválidas o captcha extra", el mismo síntoma que tuvo AMEX antes de pasar al Chrome real remoto. La lógica de `_create_driver` remoto (antes solo en `AmexScraper`) se movió a `BaseScraper._create_remote_driver` / `_create_local_driver`, y el stash de `webdriver_remote_url`/`webdriver_profile_dir`/`webdriver_headless` se movió al `run()` de la base — así cualquier scraper puede usarlo con solo declarar los campos en su config. `BbvaScraper` los hereda (y `BbvaTarjetasScraper`, que hereda de `BbvaScraper`, también). Nuevos campos "WebDriver remoto" en la config de `bbva` y `bbva_tarjetas` (factorizados en `_WEBDRIVER_REMOTO_CAMPOS`, compartido con `amex`). `AmexScraper` queda sin código propio de creación de driver — 100% heredado de la base. Si correr en remoto no alcanza (Akamai pide más que el fingerprint), el próximo paso sería sumarle interacción humanizada al login de BBVA como ya tiene AMEX.
- **DB: `busy_timeout` de 5s a 15s** (`db.py`): con el scheduler corriendo varios scrapers a la vez más un backfill grande (ej. cientos de resúmenes AMEX consolidándose de a uno), el writer podía quedar ocupado más de 5s seguidos y otros requests/scrapers tiraban `sqlite3.OperationalError: database is locked` (transitorio — no corrompe datos, se resuelve solo). Se sube el timeout de espera a 15s para tolerar mejor esos picos de escritura concurrente.
...
- **Log de versión de Chromium + flags de estabilidad + AMEX headless opcional** (`scrapers/base.py`, `scrapers/amex.py`, `scraper_credentials.py`): (1) **Log de versión**: el `_create_driver` local ahora loguea `Chromium X | chromedriver Y` en cada run (y el remoto loguea la versión del Chrome de la Mac), para correlacionar los "tab crashed" con versiones — el base image `:latest` cambia el Chromium en cada rebuild sin avisar. (2) **Flags de estabilidad** en el Chromium local (RPi): `--disable-software-rasterizer`, `--disable-extensions`, `--disable-background-networking`, `--disable-renderer-backgrounding`, `--disable-backgrounding-occluded-windows`, `--js-flags=--max-old-space-size=512`, para reducir los crashes del renderer en páginas pesadas (BBVA) con render por software. No cambian el fingerprint JS; afectan solo BBVA/MP, no AMEX (que corre en la Mac). (3) **AMEX headless**: nuevo checkbox `webdriver_headless` — corre el Chrome de la Mac sin ventana visible. Pendiente: pinear el base image a un tag fijo (cuando tengamos la versión logueada).

## 1.2.40

- **Scraper AMEX: opción de WebDriver remoto (Chrome real en una Mac) para pasar Akamai** (`scrapers/amex.py`, `scraper_credentials.py`): como Akamai bloquea el Chromium de Alpine del contenedor (RPi/ARM, sin Chrome real disponible), se agregó la opción de correr el browser en una Mac de la LAN con un Selenium server + Chrome real (fingerprint genuino que sí pasa Akamai), manejado por red. Dos campos nuevos en la config de AMEX: `webdriver_remote_url` (URL del Selenium server, ej. `http://IP-Mac:4444`) y `webdriver_profile_dir` (ruta opcional de un perfil Chrome "tibio" en la Mac). `AmexScraper` overridea `run()`/`_create_driver()`: si hay URL, usa `webdriver.Remote` con opciones mínimas (solo quita el cartel de automatización, resto fingerprint real de macOS); sin URL, usa el Chromium local de siempre. Aislado a AMEX — BBVA y MP no se tocan. Además el login ahora llena usuario/contraseña con `send_keys` (eventos de teclado reales que React e InAuth aceptan) en vez del setter JS de React.
...
- **Logs de la app visibles en el tab Log del add-on** (`log_config.json`): los `logger.info/warning/error` de los módulos de la app (scrapers, DB, scheduler, etc.) no aparecían en el output del add-on porque el `log_config.json` de uvicorn solo definía handlers para los loggers de uvicorn. Se agregó un handler `console` al root logger para que todos los logs de la app salgan por stderr con formato `[nombre_módulo]`, igual que los de uvicorn. Los access logs siguen por stdout sin cambios.
...
- **Refresco live de gastos y gráficos** (`static/app.js`): el refresco automático de widgets ahora también actualiza la tabla de gastos, gráficos, presupuesto y cuentas (usa `refreshAfterDataChange()` completo). Cuando termina un import o scraper, los nuevos gastos aparecen solos sin Ctrl-R. Si el usuario está editando una celda en la grilla de gastos, el refresco omite la tabla para no perder la edición en curso y solo actualiza saldos y vencimientos.
